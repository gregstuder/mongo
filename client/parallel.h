...
namespace mongo {
    ...
    class ParallelCursorStats {

        void logStats( int numTries ){
            scoped_lock lk( _lock );

            assert( numTries > 0 );
            _totalQueries++;
            _totalTries += numTries;

            if( _totalTries > numeric_limits<long long>::max() / 2 ){
                _totalTries /= 2;
                _totalQueries /= 2;
            }
        }
        ...
        mongo::mutex _lock;
        long long _totalTries;
        long long _totalQueries;
    };

    extern ParallelCursorStats pcursorStats;
